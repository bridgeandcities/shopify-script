...
$site = Read-Host "[1] kbdfans`n[2] novelkeys`nWhich site?`n"
$uri = switch ($site) {
    1 {
        $siteRoot = "https://kbdfans.com/collections/"
        $siteCategory = Read-Host "[1] Switches`n[2] Keycaps`n[3] Diy Kit`n[4] Group Buy`n"
        switch ($siteCategory) {
    ...
    2 {
        ...
        $siteCategory = Read-Host "[1] Switches`n[2] Keycaps`n[3] Diy Kits`n[4] Group Buys`n"
        switch ($siteCategory) {
            1 { $siteRoot + "switches" + "/products.json"  }
            2 { $siteRoot + "keycaps" + "/products.json" }
